...
BmnTrackingQaExp::BmnTrackingQaExp(Short_t ch, TString name, TString gemConf, TString silConf, TString cscConf) : FairTask("BmnTrackingQAExp", 1),
fHM(nullptr),
fReport(nullptr),
fMonitorMode(kFALSE),
fOutputDir("./"),
...
void BmnTrackingQaExp::CreateH1(const string& name, const string& xTitle, const string& yTitle, Int_t nofBins, Double_t minBin, Double_t maxBin) {
    TH1F* h = new TH1F(name.c_str(), string(name + ";" + xTitle + ";" + yTitle).c_str(), nofBins, minBin, maxBin);
    fHM->Add(name, h);
}

void BmnTrackingQaExp::CreateH2(
        const string& name,
        const string& xTitle,
        const string& yTitle,
        const string& zTitle,

...
void BmnTof2Raw2DigitNew::fillPreparation(TClonesArray *data, map<UInt_t,Long64_t> *ts, Double_t t0, Double_t t0width) {
    ...
    for (int i = 0; i < data->GetEntriesFast(); i++) {
       ...
       for(ind=0;ind<n_rec;ind++) 
         if(digit->GetSerial()==mapa[ind].id && digit->GetSlot()==mapa[ind].slot && chan==mapa[ind].chan) break;
...
//       printf("%d %d %d %d %d\n", i, mapa[ind].slot, mapa[ind].chan, digit->GetValue(), T0raw);
	...
	if (W > wmaxs[mapa[ind].plane] && ((W1 < Wcut && W2 < Wcut)||(W1 >= Wcut && W2 >= Wcut)))
	{
...
//        printf("%d %d %f %f %f\n",mapa[ind].plane,mapa[ind].strip,L*HPTIMEBIN,W,D*HPTIMEBIN);
       }
    }
}

...
// constants definition
	...
	const Double_t BmnDchHitProducer::sqrt2=sqrt(2.); // 
        ...
        const Double_t BmnDchHitProducer::clusdens_Ar=25., BmnDchHitProducer::clusdens_CO2=35.; // cluster density per track length - clusters/cm
        const Double_t BmnDchHitProducer::meanpath_Ar=1./clusdens_Ar, BmnDchHitProducer::meanpath_CO2=1./clusdens_CO2; // cluster density per track length - clusters/cm
        const Double_t BmnDchHitProducer::meanpath=0.7*meanpath_Ar+0.3*meanpath_CO2; // for 70/30 ArCO2
        const Double_t BmnDchHitProducer::clusmean=1./meanpath;
	...
	const Double_t BmnDchHitProducer::driftTimeMax=stepXYhalf/gasDriftSpeed; // 25 mkm/ns == 2.5e-3 cm/ns 
	const Double_t BmnDchHitProducer::wireDriftSpeed =  20; // 20 cm/ns
	...
	const Double_t BmnDchHitProducer::WheelR_2 = pow(MaxRadiusOfActiveVolume,2.); // cm x cm
        const UInt_t BmnDchHitProducer::nintervals=40;
        const TVector2 BmnDchHitProducer::unitU(0.,1.);
        const TVector2 BmnDchHitProducer::unitV(0.,-1.);
...
//------------------------------------------------------------------------------------------------------------------------
//BmnDchHitProducer::BmnDchHitProducer(UInt_t num = 1, Int_t verbose, Bool_t test, TString runtype, UShort_t iter) : FairTask("Dch HitProducer", verbose), fDoTest(test), fRSigma(0.2000), fRPhiSigma(0.0200), fOnlyPrimary(kFALSE)
BmnDchHitProducer::BmnDchHitProducer(Int_t verbose, Bool_t test, TString runtype) : FairTask("Dch HitProducer", verbose), fDoTest(test), fRSigma(0.2000), fRPhiSigma(0.0200), fOnlyPrimary(kFALSE)
{
...
void BmnDchHitProducer::FitHistsAngleCorr(){
     ...
     TFile *corrfile = new TFile("testtmp.BmnDchHitProducer.root","read");
...
InitStatus 		BmnDchHitProducer::Init() 
{
        ...
        for (UShort_t j = 0; j < numChambers; j++) {
         scaleSimExTDC[j]=(ranmax[j]-ranmin[j])/driftTimeMax;
        }
    ...
    //Get ROOT Manager
  	...
  	if(!fBmnDchPointsArray || !fBmnDchPointsArray2 || !fMCTracksArray){ Error("BmnDchHitProducer::Init","Branch not found!"); return kERROR; }
...
//------------------------------------------------------------------------------------------------------------------------
Double_t BmnDchHitProducer::GetDriftDist(FairMCPoint *dchPoint, UShort_t idch, UShort_t uidLocal, Double_t x, Double_t &wirePos){
...
void BmnDchHitProducer::rtCalibration(){
...
TFile *fdstread; 
if(fDchNum==1){
     fdstread = new TFile("/home/fedorisin/trunk/bmnroot/macro/run/test001.BmnDchHitProducer_calib.root","read");
}else if(fDchNum==2){
     fdstread = new TFile("/home/fedorisin/trunk/bmnroot/macro/run/test002.BmnDchHitProducer_calib.root","read");
}
...
ranmin[i]=minDriftTime->GetBinCenter(ranminbin)-0.5*(minDriftTime->GetBinWidth(ranminbin)); 
ranmax[i]=minDriftTime->GetBinCenter(hminbin2)+0.5*(minDriftTime->GetBinWidth(hminbin2)); 
...
nbinsUsed=Int_t(ranmax/binwidth)-Int_t(ranmin[i]/binwidth)+1;
}
Double_t timeRange=ranmax[i]-ranmin[i];
cout<<"time interval: "<<ranmin[i]<<" "<<ranmax[i]<<endl;
const UInt_t npoints=nintervals+1;
Double_t deldt=(ranmax[i]-ranmin[i])/Double_t(nintervals);
//Double_t dt[npoints],r[npoints];
Double_t dt[nintervals],r[nintervals];
//Double_t norm=diagonal/(fitdt->Integral(0.,ranmax));
//Double_t norm=stepXYhalf/(fitdt->Integral(0.,ranmax));
if(checkDch)cout<<"nbinsUsed = "<<nbinsUsed<<endl;
Double_t norm=stepXYhalf/SplineIntegral(ranmin[i],ranmax[i],nbinsUsed*5);
for (Int_t j = 0; j < nintervals; j++) {
 dt[j]=deldt*Double_t(j)+deldt/2.;
 //r[j]=norm*(fitdt->Integral(dt[0],dt[j]));
 r[j]=norm*SplineIntegral(dt[0]+ranmin[i],dt[j]+ranmin[i],5*TMath::Nint(Double_t(nbinsUsed)*(dt[j]-dt[0])/timeRange));
 if(checkDch)cout<<"calibration curve "<<dt[j]<<" "<<r[j]<<" "<<SplineIntegral(dt[0]+ranmin[i],dt[j]+ranmin[i],5*TMath::Nint(Double_t(nbinsUsed)*(dt[j]-dt[0])/timeRange))<<endl;
}
//calib[fDchNum-1] = new TGraph(npoints,dt,r);
for (Int_t i = 0; i < numChambers; i++) {
 calib[i] = new TGraph(nintervals,dt,r);
 //spline3rt = new TSpline3("r-t calibration curve",calib, "", 0., 0.);
 spline5rt[i] = new TSpline5("r-t calibration curve",calib[i], "", 0.,0.,0.,0.);
 //calib->Fit("pol5");
 calib[i]->SetTitle("r-t calibration curve");
 calib[i]->SetName("rtCalibCurve");
 spline5rt[i]->SetName("rtCalibSpline");
}
...
//------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::tdcInflexPoints(Double_t &inflX1,Double_t &inflX2){
...
void 			BmnDchHitProducer::Exec(Option_t* opt) 
{
	...
	Int_t nDchPoint;
	 ...
	 nDchPoint = fBmnDchPointsArray2->GetEntriesFast();
        ...
        if(checkDch)cout << " BmnDchHitProducer::Exec(), Number of BmnDchPoints = " << nDchPoint <<", event number = "<<eventNum<<", chamber = "<<idch+1<<endl;
...
///////		if( ((FairMCTrack*)pMCTracks->UncheckedAt(pPoint1->GetTrackID()))->GetMotherId() != -1)continue; // primary ONLY !!!
		...
		driftDist = GetDriftDist(dchPoint,idch,uidLocal,yRot,wirePos); // [cm]
	        if(checkDch)cout<<"driftDist1 = "<<driftDist<<endl;
                ...
                drifttimemin=drifttimemin*scaleSimExTDC[idch]+ranmin[idch];
                cout<<scaleSimExTDC[idch]<<" "<<ranmin[idch]<<endl;
                driftDist=rtCurve(drifttimemin,idch); 
...
/////		pos.SetXYZ(x, y, pPoint1->GetZ());
		...
		pHit->SetWireDelay(L);
		pHit->SetDriftTime(drifttimemin-ranmin[idch]);
...
//------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::HitFinder(UShort_t idch){ 
...
void		BmnDchHitProducer::Finish()
{
        //TString str1;
        //str1.Form("%u",Iter);
        fOutputTestFileName = TString("test00CC") + TString(".BmnDchHitProducer") + TString(".root");
...
//------------------------------------------------------------------------------------------------------------------------
BmnDchHit* BmnDchHitProducer::AddHit0(Int_t index, Int_t detID, const TVector3& posHit, const TVector3& posHitErr, Int_t trackIndex, Int_t pointIndex, Int_t flag, UInt_t dchlayer, UShort_t idch)
{
...
//------------------------------------------------------------------------------------------------------------------------
BmnDchHit* BmnDchHitProducer::AddHit(Int_t index, Int_t detID, const TVector3& posHit, const TVector3& posHitErr, Int_t trackIndex, Int_t pointIndex, Int_t flag, UInt_t dchlayer, UShort_t idch)
{
...
//-------------------------------------------------------------------------------------------------------------------------
/*DchTrackCand* BmnDchHitProducer::AddTrack(Int_t index, Int_t detID, const TVector3& posHit, const TVector3& posHitErr, Int_t trackIndex, Int_t pointIndex, Int_t flag, UInt_t dchlayer)
{
...
//-------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::CoordinateFinder(const Double_t y1,const Double_t x2,const Double_t k3,const Double_t q3,const Double_t k4,const Double_t q4,const UShort_t *ijk,Double_t x[],Double_t y[]){ 
...
//-------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::ReturnPointers(UInt_t ijk,UInt_t i,ScalarD* &hitX,ScalarI* &trackId,ScalarI* &detId,ScalarUI* &hitwire,ScalarD* &driftlen,ScalarD* &drifttim,ScalarI* &pointind){
...
Double_t BmnDchHitProducer::LineLineDCA(const Double_t x1, const Double_t y1, const Double_t z1, const Double_t dircosx1, const Double_t dircosy1, const Double_t dircosz1, const Double_t stx, const Double_t sty, const Double_t stz, const Double_t dircosstx, const Double_t dircossty, const Double_t dircosstz, Double_t &x2, Double_t &y2, Double_t &z2, Double_t &xstmin, Double_t  &ystmin, Double_t &zstmin, Double_t &length, Double_t &mu0, Double_t &lambda0){
...
//---------------------------------------------------------------------------
Double_t BmnDchHitProducer::LineLength(const Double_t x1, const Double_t y1, const Double_t z1, const Double_t x2, const Double_t y2, const Double_t z2){
...
//-------------------------------------------------------------------------------------------------------------------------
Double_t BmnDchHitProducer::rtCurve(const Double_t time, const UShort_t idch)
{

Double_t radDist;
if(checkDch)cout<<"time = "<<time<<" "<<ranmin[idch]<<" "<<ranmax<<endl;
if(time<ranmin[idch]){
radDist=0.;
...
//-------------------------------------------------------------------------------------------------------------------------
Double_t BmnDchHitProducer::SplineIntegral(const Double_t a, const Double_t b, const Int_t n)
{
...
//-------------------------------------------------------------------------------------------------------------------------
UShort_t BmnDchHitProducer::PlanesTopology(UShort_t idch){
...
if(checkDch)cout<<"planes topology in DCH "<<idch+1<<" = "<<topol<<", main planes (0 to 4) = "<<pairPlanesUsed<<", planes (0 to 8) = "<<planesUsed<<endl; 

return topol;

}
//-------------------------------------------------------------------------------------------------------------------------
UShort_t BmnDchHitProducer::RunTypeToNumber(TString runType_){
...
//-------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::ExtrapToDch(const Double_t x[],const Double_t y[],const Double_t zLayer_[][numLayers], const UShort_t idch, const UShort_t ijk[], Int_t &jjgr2){
...
//------------------------------------------------------------------------------------------------------------------------
//void	BmnDchHitProducer::rtCalibRead(){
void	BmnDchHitProducer::rtCalibCorrection(){
...
for (UShort_t j = 0; j < 2; j++) {
 ...
 calib[j]=(TGraph*)resid.Get(calibname);
 //calib[j]=(TGraph*)resid.Get("calibr");
 Int_t npoints=profResid->GetN();
 Int_t npointsCalib=calib[j]->GetN();
 ...
 Double_t* calibGrY=calib[j]->GetY();
 ...
 //calib[fDchNum-1] = new TGraph(npoints,dt,r);
 //spline3rt = new TSpline3("r-t calibration curve",calib, "", 0., 0.);
 spline5rt[j] = new TSpline5("r-t calibration curve",calib[j], "", 0.,0.,0.,0.);
 ////calib->Fit("pol5");
 //calib[fDchNum-1]->SetTitle("r-t calibration curve");
 //calib[fDchNum-1]->SetName("rtCalibCurve");
 spline5rt[j]->SetName("rtCalibSpline");

}// cycle over chambers

}
//-------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::MisAlignment(XYZPoint &xyzHit1,XYZPoint &xyzHit2,XYZPoint &xyzHit3,XYZPoint &xyzHit4,XYZPoint &xyzWire1,XYZPoint &xyzWire2,XYZPoint &xyzWire3,XYZPoint &xyzWire4){
...
//-------------------------------------------------------------------------------------------------------------------------
void BmnDchHitProducer::DchAlignment(XYZPoint &xyzHit1,XYZPoint &xyzHit2,XYZPoint &xyzHit3,XYZPoint &xyzHit4,XYZPoint &xyzWire1,XYZPoint &xyzWire2,XYZPoint &xyzWire3,XYZPoint &xyzWire4){

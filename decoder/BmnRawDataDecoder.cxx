...
BmnRawDataDecoder::BmnRawDataDecoder(TString file, TString outfile, ULong_t nEvents, ULong_t period) {
    ...
    if (fRawFileName != "") {
        ...
        fDigiFileName = (outfile == "") ? Form("bmn_run%d_digi.root", fRunId) : outfile;
        if (fRootFileName.Length()) {
            ...
            fRootFileName = Form("%sbmn_run%d_raw.root", digiPath.Data(), fRunId);
        }
    }
    fDchMapFileName = "";
    fMwpcMapFileName = "";
...
BmnStatus BmnRawDataDecoder::ConvertRawToRootIterateFile(UInt_t limit) {
    ...
    while (kTRUE) {
        ...
        if (fDat == SYNC_EOS || fDat == SYNC_STAT) {
            // read number of bytes in event
            if (fread(&fDat, kWORDSIZE, 1, fRawFileIn) != 1) continue;
            fDat = fDat / kNBYTESINWORD + (fPeriodId <= 7 ? 1 : 0); // bytes --> words
            //            printf("ev length %d\n", fDat);
...
BmnStatus BmnRawDataDecoder::ProcessEvent(UInt_t *d, UInt_t len) {
    ...
    while (idx < len) {
        ...
        if (payload + idx > len) {
            printf("Error in the event #%d: device payload length mismatch! %5u/%5u  len %5u\n", fEventId, idx, len, payload);
            return kBMNERROR;
        } else {
            if (recognized)
                idx += payload;
...
BmnStatus BmnRawDataDecoder::FillU40VE(UInt_t *d, UInt_t slot, UInt_t & idx, BmnTrigInfo* trigInfo) {
...
BmnStatus BmnRawDataDecoder::FillUT24VE_TRC(UInt_t *d, UInt_t &serial, UInt_t &len) {
    ...
    //        printf("evType %u taiFlags %u TAI %s \n",
    //                evType, ms0->TaiFlags,
    //                TTimeStamp(time_t(ms0->TaiSec), ms0->TaiNSec).AsString());
...
BmnStatus BmnRawDataDecoder::DecodeDataToDigi() {
    ...
    for (UInt_t iEv = 0; iEv < fNevents; ++iEv) {
        ...
        eventHeader->SetInputSignalsAR(headDAQ->GetInputSignalsAR());
        eventHeader->SetInputSignalsBR(headDAQ->GetInputSignalsBR());
        //        BmnTrigUnion s;
        ...
        if (curEventType == kBMNPEDESTAL) {
            ...
            timer.Start();
            if (fNdetMapper) fNdetMapper->fillEvent(tqdc_tdc, tqdc_adc, ndet);
            timer.Stop();
            rtime = timer.RealTime();
            ctime = timer.CpuTime();
...
BmnStatus BmnRawDataDecoder::InitDecoder() {
    ...
    if (fDetectorSetup.count(kNDET) > 0 && fDetectorSetup.at(kNDET) == 1) {
        printf("ndet in setup \n");
        ndet = new TClonesArray("BmnNdetDigi");
        fDigiTree->Branch("NdetDigi", &ndet);
        fNdetMapper = new BmnNdetRaw2Digit(fPeriodId, fRunId, fNdetMapFileName, fNdetCalibrationFileName);
        fNdetMapper->print();
    }
...
BmnStatus BmnRawDataDecoder::DisposeDecoder() {
    ...
    if (fNdetMapper) delete fNdetMapper;
        ...
        for (UInt_t iAdc = 0; iAdc < adcGem->GetEntriesFast(); ++iAdc) {
            ...
            for (Int_t iSer = 0; iSer < fNGemSerials; ++iSer) {
                ...
                for (UInt_t iSmpl = 0; iSmpl < adcDig->GetNSamples(); ++iSmpl) {
                    if ((fRunId > GetBoundaryRun(ADC32_N_SAMPLES)) || (fPeriodId >= 7)) {
    ...
    if (fSiliconMapper) {
        ...
        for (UInt_t iAdc = 0; iAdc < adcSil->GetEntriesFast(); ++iAdc) {
            ...
            for (auto & channelMapEl : channelMap) {
                ...
                for (UInt_t iSmpl = 0; iSmpl < adcDig->GetNSamples(); ++iSmpl) {
                    if ((fRunId > GetBoundaryRun(ADC128_N_SAMPLES)) || (fPeriodId >= 7))
                        pedData[iSer][ev][adcDig->GetChannel()][iSmpl] = (Double_t) (adcDig->GetShortValue())[iSmpl] / 16.0;
                    else
                        pedData[iSer][ev][adcDig->GetChannel()][iSmpl] = (Double_t) (adcDig->GetUShortValue())[iSmpl] / 16.0;
                    //                printf("adc = %i pedData = %f\n", (adcDig->GetShortValue())[iSmpl], pedData[iSer][ev][adcDig->GetChannel()][iSmpl]);
...
BmnStatus BmnRawDataDecoder::InitMaps() {
    ...
    BmnNdetRaw2Digit* tempNdetMapper = new BmnNdetRaw2Digit();
    tempNdetMapper->ParseConfig(fNdetMapFileName);
    fNdetSerials = tempNdetMapper->GetNdetSerials();
    fNNdetSerials = fNdetSerials.size();
    delete tempNdetMapper;
    ...
    for (auto ar : *trigArr) {
        if ((fPeriodId == 8) && (fBmnSetup == kSRCSETUP)) { // temporary crutches
            if ((fRunId >= 986) && (fRunId <= 1086)) {
                if ((strcmp(ar->GetName(), "BC2_1_A"))) continue;
            } else {

...
void BmnTof2Raw2Digit::BookSlewingResults()
{
    char name[128], title[128];
...
void BmnTof2Raw2Digit::readSlewingLimits()
{
  TString dir = getenv("VMCWORKDIR");
  TString path = dir + "/parameters/tof2_slewing/";
  char filn[128];
...
float BmnTof2Raw2Digit::slewingt0_correction(int chamber, double width, int peak)
{
    ...
    else cor = (LeadMin[chamber]+LeadMax[chamber])/2.;
    return (float)cor;
}

float BmnTof2Raw2Digit::slewing_correction(int chamber, double width, int peak)
{
    if (chamber < 0 || chamber >= MaxPlane || peak < 0 || peak > 1) return 0.;
    double cor = 0.;
...
void BmnTof2Raw2Digit::drawprep()
{
  ...
  if (MaxPlane == 15)
  {
    int champos1[] = {5,10,1,6,11,2,7,12,3,8,13,4,9,14,0};
    NDX = 5;
    NDY = 3;
    memcpy(champos, champos1, sizeof champos);
  }
  else if (MaxPlane == 24)
  {
    int champos1[] = {17,18, 3, 1,19, 4,23,20, 5,15,21, 6, 2,22, 9,10,11,12,13,14, 7, 8, 0,16};
    NDX = 8;
    NDY = 3;
    memcpy(champos, champos1, sizeof champos);
  }
  ...
  {
    int champos1[TOF2_MAX_CHAMBERS ] = {0};
    NDX = 1;
    NDY = 1;
    memcpy(champos, champos1, sizeof champos);
  }

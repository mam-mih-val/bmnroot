...
ClassImp(OnlineEventManager)

OnlineEventManager* OnlineEventManager::fgRinstance= 0;
//_____________________________________________________________________________
OnlineEventManager* OnlineEventManager::Instance()
{
  return fgRinstance;
}
//______________________________________________________________________________
OnlineEventManager::OnlineEventManager()
  :TEveEventManager("OnlineEventManager", ""),
...
void OnlineEventManager::InitColorStructure()
{
...
//______________________________________________________________________________
void OnlineEventManager::Init(Int_t visopt, Int_t vislvl, Int_t maxvisnds)
{
  TEveManager::Create();
  fRunAna->Init();
...
void OnlineEventManager::SelectedGeometryColoring()
{
...
void OnlineEventManager::RecursiveChangeNodeProperty(TGeoNode* node, Int_t color, int transparency)
{
...
void OnlineEventManager::LevelChangeNodeProperty(TGeoNode* node, int level)
{
    for(int i = 0; i < node->GetNdaughters(); i++)
    {
        TGeoNode* child = node->GetDaughter(i);
...
// yellow, orange
Int_t OnlineEventManager::GetColor(TString colorName)
{
    colorName.ToLower();
...
//______________________________________________________________________________
void OnlineEventManager::SetDepth(Float_t d)
{
    fRPhiMng->SetCurrentDepth(d);
    fRhoZMng->SetCurrentDepth(d);
}
//______________________________________________________________________________
void OnlineEventManager::ImportGeomRPhi(TEveElement* el)
{
    fRPhiMng->ImportElements(el, fRPhiGeomScene);
}
//______________________________________________________________________________
void OnlineEventManager::ImportGeomRhoZ(TEveElement* el)
{
    fRhoZMng->ImportElements(el, fRhoZGeomScene);
}
//______________________________________________________________________________
void OnlineEventManager::ImportEventRPhi(TEveElement* el)
{
    fRPhiMng->ImportElements(el, fRPhiEventScene);
}
//______________________________________________________________________________
void OnlineEventManager::ImportEventRhoZ(TEveElement* el)
{
    fRhoZMng->ImportElements(el, fRhoZEventScene);
}

//______________________________________________________________________________
void OnlineEventManager::UpdateEditor()
{
}

// OnlineEventManager destructor (empty now)
OnlineEventManager::~OnlineEventManager()
{
}

//______________________________________________________________________________
void OnlineEventManager::Open()
{
}

// go to FairRunAna event with given number for scene data getting
void OnlineEventManager::GotoEvent(Int_t event)
{
  fEntry=event;
  fRunAna->Run((Long64_t)event);
}
// go to next FairRunAna event for scene data getting
void OnlineEventManager::NextEvent()
{
  fEntry+=1;
  fRunAna->Run((Long64_t)fEntry);
}
// go to previous FairRunAna event for scene data getting
void OnlineEventManager::PrevEvent()
{
  fEntry-=1;
  fRunAna->Run((Long64_t)fEntry);
}
...
//______________________________________________________________________________
void OnlineEventManager::Close()
{
}

//______________________________________________________________________________
void OnlineEventManager::DisplaySettings()
{
}

// return integer value of color for track by particle pdg (default, white)
Int_t OnlineEventManager::Color(int pdg)
{
  switch(pdg) {
  case   22     :
    return  623;    // photon
...
void OnlineEventManager::AddParticlesToPdgDataBase(Int_t pdg)
{

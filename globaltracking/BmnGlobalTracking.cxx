...
InitStatus BmnGlobalTracking::Init() {
    ...
    if (!fSiliconTracks) cout << "BmnGlobalTracking::Init(): branch " << "BmnSiliconTrack" << " not found!" << endl;

    fDchTracks = (TClonesArray*) ioman->GetObject("BmnDchTrack");
    if (!fDchTracks) cout << "BmnGlobalTracking::Init(): branch " << "BmnDchTrack" << " not found!" << endl;
...
BmnStatus BmnGlobalTracking::MatchingSil(BmnGlobalTrack* glTr) {
    ...
    if (glTr->GetNHits() != 6) return kBMNERROR;
    ...
    for (Int_t trIdx = 0; trIdx < fSiliconTracks->GetEntriesFast(); ++trIdx) {
        ...
        kalman->TGeoTrackPropagate(&parSil, Z0, fPDG, NULL, NULL, kFALSE);
...
BmnStatus BmnGlobalTracking::MatchingDCH(BmnGlobalTrack* tr) {
    ...
    if (tr->GetNHits() != 6) return kBMNERROR;
    //
    ...
    for (Int_t trIdx = 0; trIdx < fDchTracks->GetEntriesFast(); ++trIdx) {
        ...
        kalman->TGeoTrackPropagate(&parDch, Z0, fPDG, NULL, NULL, kFALSE);

...
BmnLargeCSC::BmnLargeCSC()
: FairDetector("LargeCSC", kTRUE) {
    fPointCollection = new TClonesArray("BmnLargeCSCPoint");
    fPosIndex = 0;
    fVerboseLevel = 1;
    ResetParameters();
}

BmnLargeCSC::BmnLargeCSC(const char* name, Bool_t active)
...
Bool_t BmnLargeCSC::ProcessHits(FairVolume* vol) {
    ...
    // Create BmnLargeCSCPoint at EXIT of active volume;
	...
	fPosOut = pos - corr_step;
        ...
        BmnLargeCSCPoint *p = AddHit(fTrackID, fVolumeID,
                                fPosIn, fPosOut,
                                fMomIn, fMomOut,
        ...
        ((CbmStack*)gMC->GetStack())->AddPoint(kLargeCSC);
    }

    return kTRUE;
}

void BmnLargeCSC::EndOfEvent() {
    if(fVerboseLevel) Print();
    fPointCollection->Clear();
    fPosIndex = 0;
}

void BmnLargeCSC::Register() {
    FairRootManager::Instance()->Register("LargeCSCPoint", "LargeCSC", fPointCollection, kTRUE);
}

TClonesArray* BmnLargeCSC::GetCollection(Int_t iColl) const {
    if(iColl == 0) return fPointCollection;
    return NULL;
}

void BmnLargeCSC::Print() const {
    Int_t nHits = fPointCollection->GetEntriesFast();
    cout << "-I- BmnLargeCSC: " << nHits << " points registered in this event." << endl;
...
void BmnLargeCSC::Reset() {
    fPointCollection->Clear();
    ResetParameters();
}

void BmnLargeCSC::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset) {
    Int_t nEntries = cl1->GetEntriesFast();
    cout << "-I- BmnLargeCSC: " << nEntries << " entries to add." << endl;
    TClonesArray& clref = *cl2;
    BmnLargeCSCPoint* oldpoint = NULL;

    for(Int_t i = 0; i < nEntries; i++) {
        oldpoint = (BmnLargeCSCPoint*) cl1->At(i);
        Int_t index = oldpoint->GetTrackID() + offset;
        oldpoint->SetTrackID(index);
        new (clref[fPosIndex]) BmnLargeCSCPoint(*oldpoint);
        fPosIndex++;
    }

    cout << "-I- BmnLargeCSC: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}

void BmnLargeCSC::ConstructGeometry() {
    TString fileName = GetGeometryFileName();

    if( fileName.EndsWith(".root") ) {
        LOG(info) << "Constructing LargeCSC geometry from ROOT file " << fileName.Data();
        ConstructRootGeometry();
    }

    else if ( fileName.EndsWith(".gdml") ) {
        LOG(info) << "Constructing LargeCSC geometry from GDML file " << fileName.Data();
        ConstructGDMLGeometry();
    }

    else {
        LOG(fatal) << "Geometry format of LargeCSC file " << fileName.Data() << " not supported.";
    }
}

// -----   ConstructGDMLGeometry   ---------------------------------------------
void BmnLargeCSC::ConstructGDMLGeometry() {
    TFile *old = gFile;
    TGDMLParse parser;
    TGeoVolume* gdmlTop;
...
void BmnLargeCSC::ExpandNodeForGdml(TGeoNode* node) {
...
//--- Check if Sensitive -------------------------------------------------------
Bool_t BmnLargeCSC::CheckIfSensitive(std::string name) {
    TString tsname = name;
...
//------------------------------------------------------------------------------
BmnLargeCSCPoint* BmnLargeCSC::AddHit(Int_t trackID, Int_t detID,
    ...
    new(clref[size]) BmnLargeCSCPoint(trackID, detID,
                                        posIn, posOut, momIn, momOut,
                                        time, length, eLoss,
                                        isPrimary, charge, pdgId);

    BmnLargeCSCPoint *hit = (BmnLargeCSCPoint*) clref.At(clref.GetEntriesFast() - 1);

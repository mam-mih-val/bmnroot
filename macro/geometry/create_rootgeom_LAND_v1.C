...
/////////////////////////////////////////////////////////
// Geometry file for LAND detector @ JINR 
//       modified from R3B LAND
//
// Log           
// v1 J.Tanaka 31.10.2017
// Number of LAND planes is 5
// Veto plane included
//
// Definition : Beam direction -Z -> +Z
// The right handed cordinate on the beam axis
//
/////////////////////////////////////////////////////////
...
Double_t fTheta = 0.;
Double_t fPsi   = 0.;
Double_t fX = -33.4; //-33.4cm pm 1.0cm : Beamline to nearest active area
                    //(Beamline to the center of the active area = 137.3cm pm 1.0cm)
Double_t fY = 97.4; //97.4cm pm 0.2cm : Ground to nearest active area
                    //(Ground to the center of active area = 201.3 cm pm 0.2cm)
Double_t fZ = 0.0; //0.0cm : upstream surface of Veto is set to be Z=0.0 
//Double_t fZ = 67.5; //67.5cm pm 0.2cm : End-edge frame of DC to upstream surface of Veto 
...
void create_rootgeom_LAND_v1(const char* geoTag)
{
  ...
  const TString geoDetectorName = "LAND"; //
  ...
  double startZ=-(10.2 * 5 + 4 * spacing )/2;
  //  double startZ=-(10.2 * 5 + 9 * spacing )/2;
  double startY=-(10.2 * 20 + 19 * spacing )/2;

  // nrPlane : number of Planes (0 - 5 for SRC experiment)
  for (tz=startZ+10.2/2; nrPlane < 5; tz+=10.2+spacing)
  {
    nrPlane++;
    int j=0;
    for (ty=startY+10.2/2; j < 20; ty+=10.2+spacing)
    {
      j++;
      nrPaddle++;
      if (nrPlane % 2 == 1)
  ...
  //===================================================================================
  //------------------ Veto BC408 detectors -----------------------------------------
  
  tx=0.;
  int nrPaddle=0, nrPlane=0;
  Double_t Veto_dim1x = 100.0; // (Length of Veto = 200.0 cm)/2
  Double_t Veto_dim1y = 5.0;   // (Width of Veto = 10.0 cm)/2
  Double_t Veto_dim1z = 0.50;  // (Thickness of Veto = 1.0 cm)/2
  TGeoVolume *Veto_box1 = gGeoManager->MakeBox("Veto_box1",
						  BC408,
						  Veto_dim1x,
						  Veto_dim1y,
						  Veto_dim1z);
  TGeoVolume *aVeto = new TGeoVolumeAssembly("AVETO");
  // BC408 paddles
  xx = 0.;
  yy = 0.;
  zz = 0.;
  for (int i = 1; i < 2; i++)
  {
    aVeto->AddNode(Veto_box1,i,new TGeoCombiTrans(xx,yy,zz,zeroRotation));
    zz+=1;
  }
  // Spacing between facing surfaces of the Veto and the first LAND plane.
  double spacing_Veto = 21.6; // 21.6cm pm 0.1cm measured 31.10.2017 by J.Tanaka 
  //
  double startZ_Veto=startZ-spacing_Veto-1.0;
  //  double startZ=-(10.2 * 5 + 4 * spacing )/2;
  double startY_Veto=-(10.2 * 20 + 19 * spacing)/2;
  double startX_Veto=(10.2 * 20 + 19 * spacing)/2;

  // nrPlane : number of Planes (0 - 5 for SRC experiment)
  // tz=startZ+Veto_offset
  for (tz=startZ_Veto+1.0/2; nrPlane < 1; tz+=1.0+spacing)
  {
    nrPlane++;
    int j=0;
    for (ty=startY_Veto+10.2/2; j < 20; ty+=10.2+spacing)
    {
      j++;
      nrPaddle++;
      cell->AddNode(aVeto,nrPaddle,new TGeoCombiTrans(ty,tx,tz,rot4));
    }
  }
  //==============================================================================
  ...
  //
  // Define TOP Geometry
  //TGeoVolume* LAND = new TGeoVolumeAssembly(geoDetectorName);
  // LAND->SetMedium(pMedAir);
  // top->AddNode(LAND, 0); // 

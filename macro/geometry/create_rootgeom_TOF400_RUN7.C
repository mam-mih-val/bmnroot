...
void create_rootgeom_TOF400_RUN7() {
    ...
    const double tofZ_3box = 454.8.; // [cm] :
    const double tofZ_4box = 442.6.; // [cm] :

    double tofX = tofXgap / 2. + DetXsize / 2.;
    double tofZshift = tofZgap + DetZsize;
    double box1_x = 146.; // [cm]
    double box2_x = 90.; // [cm]
    double box3_x = -90.; // [cm]
    double box4_x = -146.; // [cm]

    // 1 box
    topTof->AddNode(vDetector, 1, new TGeoTranslation("t1", box1_x, 2. * DetYsize - 2 * tofYoverlap, tofZ_1box + tofZshift)); // right side, x > 0
    topTof->AddNode(vDetector, 2, new TGeoTranslation("t1", box1_x, 1. * DetYsize - tofYoverlap, tofZ_1box));
    topTof->AddNode(vDetector, 3, new TGeoTranslation("t1", box1_x, 0, tofZ_1box + tofZshift));
    topTof->AddNode(vDetector, 4, new TGeoTranslation("t1", box1_x, -1. * DetYsize + tofYoverlap, tofZ_1box));
    topTof->AddNode(vDetector, 5, new TGeoTranslation("t1", box1_x, -2. * DetYsize + 2 * tofYoverlap, tofZ_1box + tofZshift));

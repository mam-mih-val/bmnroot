...
void 	MakeBMNROOT_TOFv3()
{
    ...
    const TString geoDetectorName = "TOFB1";
	...
	// Air
    	...
    	TGeoMedium* medAir = gGeoMan->GetMedium("air");		if(! medAir) Fatal("Main", "Medium air not found");
	...
	TGeoMedium* medPprop = gGeoMan->GetMedium("polypropylene");	if(! medPprop) Fatal("Main", "Medium polypropylene not found");

	// lead
	FairGeoMedium* matPb = geoMedia->getMedium("lead");	if(! matPb) Fatal("Main", "FairMedium lead not found");
	geoBuild->createMedium(matPb);
	TGeoMedium* medPb = gGeoMan->GetMedium("lead");		if(! medPb) Fatal("Main", "Medium lead not found");
	...
	TGeoMedium* medAl = gGeoMan->GetMedium("aluminium");	if(! medAl) Fatal("Main", "Medium aluminium not found");
	...
	TGeoMedium* medGlass = gGeoMan->GetMedium("RPCglass");	if(! medGlass) Fatal("Main", "Medium RPCglass not found");
	...
	TGeoMedium* medGas = gGeoMan->GetMedium("RPCgas");	if(! medGas) Fatal("Main", "Medium RPCgas not found");

	// G10
	FairGeoMedium* matG10 = geoMedia->getMedium("G10");	if(! matG10) Fatal("Main", "FairMedium G10 not found");
	geoBuild->createMedium(matG10);
	TGeoMedium* medG10 = gGeoMan->GetMedium("G10");		if(! medG10) Fatal("Main", "Medium G10 not found");
	...
	TGeoBBox *sDetPlate = new TGeoBBox(detPlateX/2., detPlateY/2., detPlateZ/2.); sDetPlate->SetName("DetectorPlate");	
	...
	for(int i=0; i < NStrips; i++)
	{
    	...
    	// -------------------------------------- Install Detectors
      	const double  	tofZgap = 0.15; // 15 mm between detectors
      	const double  	tofYoverlap = 3.0; // 30 mm detectors overlap at X direction
      	
    	const double 	tofXgap = 100.; // [cm] FIXME:  dummy value now
      	const double 	tofZ = 400.; // [cm] FIXME:  dummy value now
      	
      	double tofX =   tofXgap/2. + DetXsize/2.;
      	double tofZshift = tofZgap/2. + DetZsize/2.;
      	
    	topTof->AddNode(vDetector, 1, new TGeoTranslation("t1",  +tofX,  0., tofZ + tofZshift)); // right side, x > 0
    	topTof->AddNode(vDetector, 2, new TGeoTranslation("t1",  +tofX,  +DetYsize -tofYoverlap, tofZ - tofZshift));  
    	topTof->AddNode(vDetector, 3, new TGeoTranslation("t1",  +tofX,  -DetYsize +tofYoverlap, tofZ - tofZshift));      	  
    
      	topTof->AddNode(vDetector, 4, new TGeoTranslation("t1",  -tofX,  0., tofZ + tofZshift)); // right side, x < 0

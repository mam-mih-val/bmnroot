...
//    chainDST->SetBranchAddress("BmnSiliconTrack", &gemTracks);
    ...
    TF1 *funcRescale = BmnRecoTools::GetRescaleFunc(TString("RescaleCSC"), mc, ex);
    ...
    hGemEx->SetMinimum(1e-4);
    hGemEx->SetMaximum(9e-2);
    hGemEx->Draw();
...
//    legend->AddEntry(hGemExTracks, "Digits from tracks");
    ...
    can->Print(Form("%s-th-%.1f-cs-%d-Lower-2m.pdf", can->GetName(), lowThr, ClusterSizeThr));
    can->Clear();

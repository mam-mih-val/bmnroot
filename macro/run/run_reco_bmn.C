...
{   // Verbosity level (0=quiet, 1=event-level, 2=track-level, 3=debug)
    ...
    if (digiFileName.Contains("bmn_run")) {
        // A new, more flexible and robust, extraction of run period and run number of the file.
        // Anatoly.Solomin@jinr.ru 2017-02-21 15:14:20
        ...
        // ---------------------------------------------------------------------
        if (digiFileName.BeginsWith("run") && digiFileName.Contains(":")) {
            // that is when 'run[0]*N-[0]*NNN:'-like prefix is used
            ...
            substr = substr(   "[0-9]+");
            Int_t    run_period = substr.Atoi();
            // get run number from '-NNN'
            substr = prefix("-[0-9]+");
            substr = substr( "[0-9]+");
            Int_t    run_number = substr.Atoi();
            // and now strip digiFileName from the 'run[0]*N-[0]*NNN:' prefix
            digiFileName.ReplaceAll(prefix, ""); }
        // ---------------------------------------------------------------------
        // The following, for now, is used only in the alignment, when file name
        // is like 'bmn_run005_Glob_000812_digi.root'.
        // It will be changed as soon as the new standard file naming is agreed
        // upon.
        else if (digiFileName.Contains(re)) {
            // get run period
            ...
            substr = substr(       "[0-9]+");
            Int_t    run_number = substr.Atoi(); }
        // Anatoly.Solomin@jinr.ru 2017-02-21 16:27:45
        // ---------------------------------------------------------------------
        else {
            cout <<"Error: digi file name "+digiFileName+" is not recognized!"<< endl;
            exit(-1);
        }
        // check existence of the actual input digi file (prefix already stripped from name):

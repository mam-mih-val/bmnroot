...
const Float_t Z_dch1 = 538.2; //z coord in the midle of dch1	
const Float_t Z_dch2 = 737.2; //z coord in the midle of dch2
...
const Float_t x1_sh = 8.11; //
const Float_t x2_sh = 1.69; //
const Float_t y1_sh = -4.23;
const Float_t y2_sh = -6.0;
...
const Float_t x1_slope_sh = -0.0944;
const Float_t y1_slope_sh = 0.045; //was .05
const Float_t x2_slope_sh = -0.0937;
const Float_t y2_slope_sh = 0.052; //
...
void recoRunDCH_VP(Int_t runId = 1700) {
    ...
    //Int_t rcode = bmnTree->Add(TString::Format("../../raw/bmn_run%04d_digi.root", runId),0);
    Int_t rcode = bmnTree->Add(TString::Format("/nica/mpd16/bmndata3/run6/root/digi/bmn_run%04d_digi.root", runId),0);
    if ( rcode == 0)
    {
	delete bmnTree;
	bmnTree = new TChain("BMN_ALLDIGIT");
	rcode = bmnTree->Add(TString::Format("bmn_run%04d_digi.root", runId),0);
	if (rcode == 0)
	{
	    delete bmnTree;
	    printf("\"cbmsim\" and \"BMN_ALLDIGIT\" trees not found in file %s\n", (TString::Format("bmn_run%04d_digi.root", runId)).Data());
	    return;
	}
	else
	{
	    printf("\"BMN_ALLDIGIT\" tree found in file %s\n", (TString::Format("bmn_run%04d_digi.root", runId)).Data());
	};
    }
    else
    {
	printf("\"cbmsim\" tree found in file %s\n", (TString::Format("bmn_run%04d_digi.root", runId)).Data());
    };
    ...
    ifstream fin;
    fin.open(TString::Format("pol_coord0%d.txt", runId), ios::in);
    ...
    for (Int_t iEv = startEvent; iEv < startEvent + nEvents; iEv++) {
        ...
        if (has7DC1) {
            ...
            for (int segdc1Nr = 0; segdc1Nr < nDC1_segments; segdc1Nr++) {
                ...
                if (fabs(dx) < 18 && fabs(dy) < 15) {
                    if (fabs(daX) < 0.03 && fabs(daY) < 0.03) {
                        seg_it++;
                        leng[seg_it] = sqrt(Z_dch_mid * Z_dch_mid + (xMean * xMean));
                        pair_id[seg_it] = 1000 * (segdc1Nr + 1) + match_dc2_seg + 1;
                        x_mid[seg_it] = xMean;
                        y_mid[seg_it] = yMean;
                        //a_X[seg_it] = atan(ax);
                        //a_Y[seg_it] = atan(ay);
            ...
            for (int segdc2Nr = 0; segdc2Nr < nDC2_segments; segdc2Nr++) {
                ...
                if (fabs(dx) < 18 && fabs(dy) < 15) {
                    if (fabs(daX) < 0.03 && fabs(daY) < 0.03) {
                        ...
                        seg_it++;
                        leng[seg_it] = sqrt(Z_dch_mid * Z_dch_mid + (xMean * xMean));
                        pair_id[seg_it] = 1000 * (match_dc1_seg + 1) + segdc2Nr + 1;
                        x_mid[seg_it] = xMean;
                        y_mid[seg_it] = yMean;
                        a_X[seg_it] = atan(ax);
                        a_Y[seg_it] = atan(ay);
        ...
        if (seg_it>-1) {
            for (int seg = 0; seg < seg_it + 1; seg++) {
                ...
                newTrack->SetLength(leng[seg]);
                newTrack->SetB(0.3 * intBL / a_X[seg]);
                newTrack->SetFlag(pair_id[seg]);

                //	cout<<" slope "<<a_X[seg]<<" momentum "<<(0.3*intBL/a_X[seg])<<endl;
            }
        }//seg_it>0

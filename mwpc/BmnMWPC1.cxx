...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC1::BmnMWPC1() 
 : FairDetector("MWPC1", kTRUE)
{
	fPointCollection = new TClonesArray("BmnMWPC1Point");
	fPosIndex = 0;
	fVerboseLevel = 1;
	ResetParameters();
}
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC1::BmnMWPC1(const char* name, Bool_t active)
...
//------------------------------------------------------------------------------------------------------------------------
int BmnMWPC1::DistAndPoints(TVector3 p3, TVector3 p4, TVector3& pa, TVector3& pb) {                                         
    pa=(p3+p4)*0.5;
    pb=pa;
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC1::GlobalToLocal(TVector3& global) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC1::LocalToGlobal(TVector3& local) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//----------------------------------------------------------------------------------------------------------------------
Bool_t  BmnMWPC1::ProcessHits(FairVolume* vol)
{
  ...
  // Create BmnMWPC1Point at EXIT of active volume;
  if ((gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) && fELoss > 0) {
    ...
    BmnMWPC1Point *p = AddHit(fTrackID, fVolumeID, fPos, fPos.Perp(),
                TVector3(fMom.Px(), fMom.Py(), fMom.Pz()),
                fTime, (fLength+gMC->TrackLength())/2, fELoss,
                fIsPrimary, fCharge, fPdgId, trackPosition);
    p->SetPhi(phi); //AZ

    ((CbmStack*)gMC->GetStack())->AddPoint(kMWPC1);
  }

  return kTRUE;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::EndOfEvent() 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::Register(){ FairRootManager::Instance()->Register("MWPC1Point", "MWPC1", fPointCollection, kTRUE); }
//------------------------------------------------------------------------------------------------------------------------
TClonesArray* BmnMWPC1::GetCollection(Int_t iColl) const 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::Print() const 
{
	Int_t nHits = fPointCollection->GetEntriesFast();
	cout << "-I- BmnMWPC1: " << nHits << " points registered in this event." << endl;
	
	if(fVerboseLevel > 1)
    		for(Int_t i=0; i<nHits; i++) (*fPointCollection)[i]->Print();
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::Reset(){ fPointCollection->Clear(); ResetParameters(); }
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
	Int_t nEntries = cl1->GetEntriesFast();
	cout << "-I- BmnMWPC1: " << nEntries << " entries to add." << endl;
	TClonesArray& clref = *cl2;
	BmnMWPC1Point* oldpoint = NULL;
	
	for(Int_t i=0; i<nEntries; i++) 
	{
		oldpoint = (BmnMWPC1Point*) cl1->At(i);
		Int_t index = oldpoint->GetTrackID() + offset;
		oldpoint->SetTrackID(index);
		new (clref[fPosIndex]) BmnMWPC1Point(*oldpoint);
		fPosIndex++;
	}
	
	cout << "-I- BmnMWPC1: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC1::ConstructGeometry() 
{
  TString fileName = GetGeometryFileName();
  
  if ( fileName.EndsWith(".root") ) {
    gLogger->Info(MESSAGE_ORIGIN,
                "Constructing MWPC1 geometry from ROOT file %s", 
                fileName.Data());
    ConstructRootGeometry();
  }
  else if ( fileName.EndsWith(".geo") ) {
    gLogger->Info(MESSAGE_ORIGIN,
		  "Constructing MWPC1 geometry from ASCII file %s", 
		  fileName.Data());
...
// -----   ConstructAsciiGeometry   -------------------------------------------
void BmnMWPC1::ConstructAsciiGeometry() {
  
  FairGeoLoader*    geoLoad = FairGeoLoader::Instance();
  FairGeoInterface* geoFace = geoLoad->getGeoInterface();
  BmnMWPC1Geo*       MWPC1Geo  = new BmnMWPC1Geo();
  MWPC1Geo->setGeomFile(GetGeometryFileName());
  geoFace->addGeoModule(MWPC1Geo);

  Bool_t rc = geoFace->readSet(MWPC1Geo);
  if (rc) MWPC1Geo->create(geoLoad->getGeoBuilder());
  TList* volList = MWPC1Geo->getListOfVolumes();
  // store geo parameter
  FairRun *fRun = FairRun::Instance();
  FairRuntimeDb *rtdb= FairRun::Instance()->GetRuntimeDb();
  BmnMWPC1GeoPar* par=(BmnMWPC1GeoPar*)(rtdb->getContainer("BmnMWPC1GeoPar"));
...
/*
// -----   ConstructGDMLGeometry   -------------------------------------------
void BmnMWPC1::ConstructGDMLGeometry()
{
    TGDMLParse parser;
...
void BmnMWPC1::ExpandNodeForGdml(TGeoNode* node)
{
...
//Check if Sensitive-----------------------------------------------------------
Bool_t BmnMWPC1::CheckIfSensitive(std::string name)
{
  TString tsname = name;
...
//  if(0 == TString(name).CompareTo("MWPC1DetV")) {
//    return kTRUE;
//  }
//  return kFALSE;
...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC1Point* BmnMWPC1::AddHit(Int_t trackID, Int_t detID, TVector3 pos, Double_t radius,
			    ...
			    Double_t eLoss, Int_t isPrimary, Double_t charge, Int_t pdgId, TVector3 trackPos) 
{
    ...
    return new(clref[size]) BmnMWPC1Point(trackID, detID, pos, radius, mom, time, length, eLoss, isPrimary, charge, pdgId, trackPos);
}
//------------------------------------------------------------------------------------------------------------------------

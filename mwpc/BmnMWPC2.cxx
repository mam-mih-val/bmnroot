...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC2::BmnMWPC2() 
 : FairDetector("MWPC2", kTRUE)
{
	fPointCollection = new TClonesArray("BmnMWPC2Point");
	fPosIndex = 0;
	fVerboseLevel = 1;
	ResetParameters();
}
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC2::BmnMWPC2(const char* name, Bool_t active)
...
//------------------------------------------------------------------------------------------------------------------------
int BmnMWPC2::DistAndPoints(TVector3 p3, TVector3 p4, TVector3& pa, TVector3& pb) {                                         
    pa=(p3+p4)*0.5;
    pb=pa;
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC2::GlobalToLocal(TVector3& global) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC2::LocalToGlobal(TVector3& local) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//----------------------------------------------------------------------------------------------------------------------
Bool_t  BmnMWPC2::ProcessHits(FairVolume* vol)
{
  ...
  // Create BmnMWPC2Point at EXIT of active volume;
  if ((gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) && fELoss > 0) {
    ...
    BmnMWPC2Point *p = AddHit(fTrackID, fVolumeID, fPos, fPos.Perp(),
                TVector3(fMom.Px(), fMom.Py(), fMom.Pz()),
                fTime, (fLength+gMC->TrackLength())/2, fELoss,
                fIsPrimary, fCharge, fPdgId, trackPosition);
    p->SetPhi(phi); //AZ

    ((CbmStack*)gMC->GetStack())->AddPoint(kMWPC2);
  }

  return kTRUE;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::EndOfEvent() 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::Register(){ FairRootManager::Instance()->Register("MWPC2Point", "MWPC2", fPointCollection, kTRUE); }
//------------------------------------------------------------------------------------------------------------------------
TClonesArray* BmnMWPC2::GetCollection(Int_t iColl) const 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::Print() const 
{
	Int_t nHits = fPointCollection->GetEntriesFast();
	cout << "-I- BmnMWPC2: " << nHits << " points registered in this event." << endl;
	
	if(fVerboseLevel > 1)
    		for(Int_t i=0; i<nHits; i++) (*fPointCollection)[i]->Print();
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::Reset(){ fPointCollection->Clear(); ResetParameters(); }
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
	Int_t nEntries = cl1->GetEntriesFast();
	cout << "-I- BmnMWPC2: " << nEntries << " entries to add." << endl;
	TClonesArray& clref = *cl2;
	BmnMWPC2Point* oldpoint = NULL;
	
	for(Int_t i=0; i<nEntries; i++) 
	{
		oldpoint = (BmnMWPC2Point*) cl1->At(i);
		Int_t index = oldpoint->GetTrackID() + offset;
		oldpoint->SetTrackID(index);
		new (clref[fPosIndex]) BmnMWPC2Point(*oldpoint);
		fPosIndex++;
	}
	
	cout << "-I- BmnMWPC2: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC2::ConstructGeometry() 
{
  TString fileName = GetGeometryFileName();
  
  if ( fileName.EndsWith(".root") ) {
    gLogger->Info(MESSAGE_ORIGIN,
                "Constructing MWPC2 geometry from ROOT file %s", 
                fileName.Data());
    ConstructRootGeometry();
  }
  else if ( fileName.EndsWith(".geo") ) {
    gLogger->Info(MESSAGE_ORIGIN,
		  "Constructing MWPC2 geometry from ASCII file %s", 
		  fileName.Data());
  ...
  {
    gLogger->Info(MESSAGE_ORIGIN,
          "Constructing MWPC geometry from GDML file %s",
          fileName.Data());
  ...
  else
  {
    gLogger->Fatal(MESSAGE_ORIGIN,
           "Geometry format of MWPC file %s not supported.",
           fileName.Data());
  }
}

// -----   ConstructAsciiGeometry   -------------------------------------------
void BmnMWPC2::ConstructAsciiGeometry() {
  
  FairGeoLoader*    geoLoad = FairGeoLoader::Instance();
  FairGeoInterface* geoFace = geoLoad->getGeoInterface();
  BmnMWPC2Geo*       MWPC2Geo  = new BmnMWPC2Geo();
  MWPC2Geo->setGeomFile(GetGeometryFileName());
  geoFace->addGeoModule(MWPC2Geo);

  Bool_t rc = geoFace->readSet(MWPC2Geo);
  if (rc) MWPC2Geo->create(geoLoad->getGeoBuilder());
  TList* volList = MWPC2Geo->getListOfVolumes();
  // store geo parameter
  FairRun *fRun = FairRun::Instance();
  FairRuntimeDb *rtdb= FairRun::Instance()->GetRuntimeDb();
  BmnMWPC2GeoPar* par=(BmnMWPC2GeoPar*)(rtdb->getContainer("BmnMWPC2GeoPar"));
...
/*
// -----   ConstructGDMLGeometry   -------------------------------------------
void BmnMWPC2::ConstructGDMLGeometry()
{
    TGDMLParse parser;
...
void BmnMWPC2::ExpandNodeForGdml(TGeoNode* node)
{
...
//Check if Sensitive-----------------------------------------------------------
Bool_t BmnMWPC2::CheckIfSensitive(std::string name)
{
  TString tsname = name;
...
//  if(0 == TString(name).CompareTo("MWPC2DetV")) {
//    return kTRUE;
//  }
//  return kFALSE;
...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC2Point* BmnMWPC2::AddHit(Int_t trackID, Int_t detID, TVector3 pos, Double_t radius,
			    ...
			    Double_t eLoss, Int_t isPrimary, Double_t charge, Int_t pdgId, TVector3 trackPos) 
{
    ...
    return new(clref[size]) BmnMWPC2Point(trackID, detID, pos, radius, mom, time, length, eLoss, isPrimary, charge, pdgId, trackPos);
}
//------------------------------------------------------------------------------------------------------------------------

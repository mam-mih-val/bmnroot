...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC3::BmnMWPC3() 
 : FairDetector("MWPC3", kTRUE)
{
	fPointCollection = new TClonesArray("BmnMWPC3Point");
	fPosIndex = 0;
	fVerboseLevel = 1;
	ResetParameters();
}
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC3::BmnMWPC3(const char* name, Bool_t active)
...
//------------------------------------------------------------------------------------------------------------------------
int BmnMWPC3::DistAndPoints(TVector3 p3, TVector3 p4, TVector3& pa, TVector3& pb) {                                         
    pa=(p3+p4)*0.5;
    pb=pa;
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC3::GlobalToLocal(TVector3& global) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//------------------------------------------------------------------------------------------------------------------------
TVector3 BmnMWPC3::LocalToGlobal(TVector3& local) {
    Double_t globPos[3];
    Double_t localPos[3];
...
//----------------------------------------------------------------------------------------------------------------------
Bool_t  BmnMWPC3::ProcessHits(FairVolume* vol)
{
  ...
  // Create BmnMWPC3Point at EXIT of active volume;
  if ((gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) && fELoss > 0) {
    ...
    BmnMWPC3Point *p = AddHit(fTrackID, fVolumeID, fPos, fPos.Perp(),
                TVector3(fMom.Px(), fMom.Py(), fMom.Pz()),
                fTime, (fLength+gMC->TrackLength())/2, fELoss,
                fIsPrimary, fCharge, fPdgId, trackPosition);
    p->SetPhi(phi); //AZ

    ((CbmStack*)gMC->GetStack())->AddPoint(kMWPC3);
  }

  return kTRUE;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::EndOfEvent() 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::Register(){ FairRootManager::Instance()->Register("MWPC3Point", "MWPC3", fPointCollection, kTRUE); }
//------------------------------------------------------------------------------------------------------------------------
TClonesArray* BmnMWPC3::GetCollection(Int_t iColl) const 
{
...
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::Print() const 
{
	Int_t nHits = fPointCollection->GetEntriesFast();
	cout << "-I- BmnMWPC3: " << nHits << " points registered in this event." << endl;
	
	if(fVerboseLevel > 1)
    		for(Int_t i=0; i<nHits; i++) (*fPointCollection)[i]->Print();
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::Reset(){ fPointCollection->Clear(); ResetParameters(); }
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
	Int_t nEntries = cl1->GetEntriesFast();
	cout << "-I- BmnMWPC3: " << nEntries << " entries to add." << endl;
	TClonesArray& clref = *cl2;
	BmnMWPC3Point* oldpoint = NULL;
	
	for(Int_t i=0; i<nEntries; i++) 
	{
		oldpoint = (BmnMWPC3Point*) cl1->At(i);
		Int_t index = oldpoint->GetTrackID() + offset;
		oldpoint->SetTrackID(index);
		new (clref[fPosIndex]) BmnMWPC3Point(*oldpoint);
		fPosIndex++;
	}
	
	cout << "-I- BmnMWPC3: " << cl2->GetEntriesFast() << " merged entries."  << endl;
}
//------------------------------------------------------------------------------------------------------------------------
void BmnMWPC3::ConstructGeometry() 
{
  TString fileName = GetGeometryFileName();
  
  if ( fileName.EndsWith(".root") ) {
    gLogger->Info(MESSAGE_ORIGIN,
                "Constructing MWPC3 geometry from ROOT file %s", 
                fileName.Data());
    ConstructRootGeometry();
  }
  else if ( fileName.EndsWith(".geo") ) {
    gLogger->Info(MESSAGE_ORIGIN,
		  "Constructing MWPC3 geometry from ASCII file %s", 
		  fileName.Data());
...
// -----   ConstructAsciiGeometry   -------------------------------------------
void BmnMWPC3::ConstructAsciiGeometry() {
  
  FairGeoLoader*    geoLoad = FairGeoLoader::Instance();
  FairGeoInterface* geoFace = geoLoad->getGeoInterface();
  BmnMWPC3Geo*       MWPC3Geo  = new BmnMWPC3Geo();
  MWPC3Geo->setGeomFile(GetGeometryFileName());
  geoFace->addGeoModule(MWPC3Geo);

  Bool_t rc = geoFace->readSet(MWPC3Geo);
  if (rc) MWPC3Geo->create(geoLoad->getGeoBuilder());
  TList* volList = MWPC3Geo->getListOfVolumes();
  // store geo parameter
  FairRun *fRun = FairRun::Instance();
  FairRuntimeDb *rtdb= FairRun::Instance()->GetRuntimeDb();
  BmnMWPC3GeoPar* par=(BmnMWPC3GeoPar*)(rtdb->getContainer("BmnMWPC3GeoPar"));
...
/*
// -----   ConstructGDMLGeometry   -------------------------------------------
void BmnMWPC3::ConstructGDMLGeometry()
{
    TGDMLParse parser;
...
void BmnMWPC3::ExpandNodeForGdml(TGeoNode* node)
{
...
//Check if Sensitive-----------------------------------------------------------
Bool_t BmnMWPC3::CheckIfSensitive(std::string name)
{
  TString tsname = name;
...
//  if(0 == TString(name).CompareTo("MWPC3DetV")) {
//    return kTRUE;
//  }
//  return kFALSE;
...
//------------------------------------------------------------------------------------------------------------------------
BmnMWPC3Point* BmnMWPC3::AddHit(Int_t trackID, Int_t detID, TVector3 pos, Double_t radius,
			    ...
			    Double_t eLoss, Int_t isPrimary, Double_t charge, Int_t pdgId, TVector3 trackPos) 
{
    ...
    return new(clref[size]) BmnMWPC3Point(trackID, detID, pos, radius, mom, time, length, eLoss, isPrimary, charge, pdgId, trackPos);
}
//------------------------------------------------------------------------------------------------------------------------

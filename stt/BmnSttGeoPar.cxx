...
ClassImp(BmnSttGeoPar)

BmnSttGeoPar::BmnSttGeoPar(const char* name,const char* title,const char* context)
...
BmnSttGeoPar::~BmnSttGeoPar(void) {
}

void BmnSttGeoPar::clear(void) {
    if(fGeoSensNodes) delete fGeoSensNodes;
    if(fGeoPassNodes) delete fGeoPassNodes;
    fGeoSensNodes = fGeoPassNodes = 0x0; //AZ
}

void BmnSttGeoPar::putParams(FairParamList* l) {
  if (!l) return;
...
Bool_t BmnSttGeoPar::getParams(FairParamList* l) {
    if (!l) return kFALSE;

...
void add_Tof1_mapping(string placement_map_filename, string main_map_filename, int sP = 4, int sR = 61, int eP = 4, int eR = 84) {
	...
	if(ff.fail()) {cout << "Failed to open " << placement_map_filename << endl; return;}
	while(!ff.eof()) {
		ff >> std::hex >> fcrate >> std::dec >> fslot >> std::hex >> fserial;
		if(ff.eof() || fcrate==0) {break;}
	...
	int nTDC = nLines; map1Size = nTDC*3;
	...
	if(ff.fail()) {cout << "Failed to open " << main_map_filename << endl; return;}
	while(!ff.eof()) {
		ff >> std::hex >> fserial >> std::dec >> fchan >> fplane >> fstrip >> side_c;
		if(ff.eof() || fserial==0) {break;}
		if(side_c != 'L' && side_c != 'R') {cout << "Wrong mapping file: Side is not L or R." << endl;}
	...
	cin >> anwser; if(anwser == 'y' || anwser == 'Y') {
		...
		if(t==NULL) {cout << "Cannot upload the placement map" << endl; return;}
		delete t;
	}
	...
	if(anwser == 'y' || anwser == 'Y') {
		ff.open("/home/sx107/Work/JINR/root/map.txt", std::fstream::in);
		while(!ff.eof()) {
			cout << "Uploading line " << nLines << endl;
			ff >> std::hex >> fserial >> std::dec >> fchan >> fplane >> fstrip >> side_c;
			if(side_c == 'L') {side = true;}
			else if (side_c == 'R') {side = false;}
			else {cout << "Error: side is not L or R" << endl; return;}
			...
			if(t==0x00) {cout << "Cannot upload the placement map" << endl; break;}
			else {delete t;}

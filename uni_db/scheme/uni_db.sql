...
DECLARE
  bytea_size integer; i integer;
  r record;
  bool_value boolean; int_value integer; uint_value uint; double_value double precision = 0.0; text_value text = '';
BEGIN
  ...
  WHEN 3 THEN
    loop
      IF number_limit <> 0 THEN
        ...
        END IF;
      END IF;
      text_value = text_value || GetDouble4Bytea(par_value, is_little_endian, false) || ' ';
